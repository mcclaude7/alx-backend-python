...
class TestGithubOrgClient(unittest.TestCase):
    ...
    @parameterized.expand([
        (
            "google",
            TEST_PAYLOAD[0][0],  # org_payload
            TEST_PAYLOAD[0][1],  # repos_payload
            TEST_PAYLOAD[0][2]   # expected_repos
        )
    ])
    @patch('client.get_json')
    def test_public_repos(self, org_name: str, org_payload: dict,
                          repos_payload: list, expected_repos: list,
                          mock_get_json) -> None:
        """
        Test GithubOrgClient.public_repos returns the expected repos list.

        Args:
            org_name: Name of the organization.
            org_payload: Mocked organization payload with repos_url.
            repos_payload: Mocked repositories payload.
        ...
        with patch.object(GithubOrgClient, '_public_repos_url',
            ...
            # Assert results match expected repos
            self.assertEqual(result, expected_repos)
